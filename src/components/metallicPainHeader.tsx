...
function renderTextToImageData(
    primaryText: string,
    ...
    ctx.clearRect(0, 0, textCanvasWidth, textCanvasHeight);
    ...
    if (secondaryText) {
        // Two-line layout
        const primarySize = Math.round(textCanvasHeight * 0.5);
        const secondarySize = Math.round(textCanvasHeight * 0.13);
        ctx.font = `bold ${primarySize}px "ITC Benguiat", "Playfair Display", Georgia, serif`;
        ctx.fillText(primaryText, textCanvasWidth / 2, textCanvasHeight / 2 - textCanvasHeight * 0.1);
        ctx.font = `bold ${secondarySize}px "ITC Benguiat", "Playfair Display", Georgia, serif`;
        ctx.fillText(secondaryText, textCanvasWidth / 2, textCanvasHeight / 2 + textCanvasHeight * 0.28);
    } else {
        // Single text, centered
        const primarySize = Math.round(textCanvasHeight * 0.6);
        ctx.font = `bold ${primarySize}px "ITC Benguiat", "Playfair Display", Georgia, serif`;
        ctx.fillText(primaryText, textCanvasWidth / 2, textCanvasHeight / 2);
    }

    return ctx.getImageData(0, 0, textCanvasWidth, textCanvasHeight);
}

function processImage(rawData: ImageData): ImageData {
...
export default function MetallicPaintText({
    primaryText = 'VM',
    secondaryText,
    textCanvasWidth = 400,
    ...
    // Render text and upload as texture
    useEffect(() => {
        if (!ready) return;

        setTextureReady(false);
        const rawImageData = renderTextToImageData(primaryText, secondaryText, textCanvasWidth, textCanvasHeight);
        const processedData = processImage(rawImageData);
        uploadTexture(processedData);
        setTextureReady(true);
    }, [ready, uploadTexture, primaryText, secondaryText, textCanvasWidth, textCanvasHeight]);
    ...
    useEffect(() => {
        ...
        ready, seed, scale, refraction, blur, liquid, brightness, contrast,
